...
public class Table_Code extends _ParentDAO {

    private String DELETE_CODE_SQL =
            "DELETE FROM Code";
    private String INSERT_CODE_SQL =
            "INSERT INTO Code "
            + "( CodeID, Description_EN, IconName ) "
            + "VALUES ( ?, ?, ? )";
    ...
    public void clearTable() {
        ...
        try {
            ...
            preparedStatement = connection.prepareStatement( DELETE_CODE_SQL );
    ...
    public void loadTable() {
        ...
        try {
            ...
            //------------------------
            FileReader fileReader = new FileReader( SESSION_TBLXTR_PATH + SESSION_FILE_SEPARATOR + "code.txt" );
            ...
            while( line != null && ! line.trim().equals(EMPTY_STRING) ) {
                ...
                String codeID = line.substring( 0, fs01 ).trim();

                int fs02 = line.indexOf( FIELD_SEPARATOR, fs01 + 1 );
                String description_EN = line.substring( fs01 + 1, fs02 ).trim();

                String iconName = line.substring( fs02 + 1 ).trim();

                //----
                preparedStatement = connection.prepareStatement( INSERT_CODE_SQL );

                preparedStatement.setString( 1, codeID );
                preparedStatement.setString( 2, description_EN );
                preparedStatement.setString( 3, iconName );
