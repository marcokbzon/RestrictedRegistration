...
public class Table_ServiceType extends _ParentDAO {

    private String DELETE_SERVICETYPE_SQL =
            "DELETE FROM ServiceType";
    private String INSERT_SERVICETYPE_SQL =
            "INSERT INTO ServiceType "
            + "( ServiceTypeID, Abbreviation, Name_EN ) "
            + "VALUES ( ?, ?, ? )";

    public Table_ServiceType() {
    }
    ...
    public void loadTable() {
        ...
        try {
            ...
            //------------------------
            FileReader fileReader = new FileReader( SESSION_TBLXTR_PATH + SESSION_FILE_SEPARATOR + "servicetype.txt" );
            ...
            while( line != null && ! line.trim().equals(EMPTY_STRING) ) {
                ...
                String serviceTypeID = line.substring( 0, fs01 ).trim();

                int fs02 = line.indexOf( FIELD_SEPARATOR, fs01 + 1 );
                String name_EN = line.substring( fs01 + 1, fs02 ).trim();

                String abbreviation = line.substring( fs02 + 1 ).trim();

                //----
                preparedStatement = connection.prepareStatement( INSERT_SERVICETYPE_SQL );

                preparedStatement.setString( 1, serviceTypeID );
                preparedStatement.setString( 2, abbreviation );
                preparedStatement.setString( 3, name_EN );
            ...
            System.out.println( "\nSTATUS FOR TABLE LOAD : ServiceType" );
            System.out.println( "-----------------------------------------------------------" );
