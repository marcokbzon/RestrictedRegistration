...
public class AppReviewAppHistoryBean extends AbstractBean {
    ...
    public AppReviewAppHistoryBean() {
        ListBbDAO listDAO = new ListBbDAO();

        String universityID = ( String ) getFromSession( SESSION_UNIVERSITYID_PAST );
        setEmail( ( String ) getFromSession( SESSION_EMAIL ) );

        List<AppReviewAppHistoryData> applicationList = listDAO.getAppReviewAppHistoryList( universityID, getEmail() );

        Iterator<AppReviewAppHistoryData> iter = applicationList.iterator();
